...
import com.example.pharmkare.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
        ...
        user!!.updateProfile(profileUpdates)
            .addOnCompleteListener { task ->
                if (task.isSuccessful){
                    Toast.makeText(this,"Se realizaron los cambios correctamente.",Toast.LENGTH_SHORT).show()
                    updateUI()
                }
            }
    }
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        this.menuInflater.inflate(R.menu.menu_pharm, menu)
        this.menu = menu
        return true
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.opcion1 -> {
                ...
                val intent: Intent = Intent(this,MedicamentosActivity::class.java)
                startActivity(intent)
                return true
            }
            R.id.opcion2 -> {
                // manejar la opción 2 seleccionada

                val intent: Intent = Intent(this,DetalleCompraActivity::class.java)
                startActivity(intent)
               return true
            }
            R.id.opcion3 -> {
                // manejar la opción 3 seleccionada
                val intent: Intent = Intent(this,HistorialCompraActivity::class.java)
                startActivity(intent)
                return true
            }
